...
module Lib (
  ...
  linearInterpolation,
  processInput, parseCommandLineArguments, readCoordinatePair
) where
...
data Configuration where
  Configuration :: {interpolationMethods :: [String], stepSize :: Double} ->
              Configuration
...
parseCommandLineArguments args = Configuration {
